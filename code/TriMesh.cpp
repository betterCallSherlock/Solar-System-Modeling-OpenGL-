...
void TriMesh::readMtl(const std::string& mtlFilename)
{
    ...
    if (!fin)
    {
        std::cout << "ERROR: cannot open the MTL file: " << mtlFilename << std::endl;
        return;
    }
    ...
    while (std::getline(fin, line))
    {
        ...
        if (type == "newmtl")
        {
            ...
            {
                materials.push_back(material); // 保存上一个材质
            }

            sin >> material.name;  // 读取新的材质名称
            material.ambient = glm::vec3(0.2f, 0.2f, 0.2f); // 默认环境光
            material.diffuse = glm::vec3(0.8f, 0.8f, 0.8f); // 默认漫反射
            material.specular = glm::vec3(1.0f, 1.0f, 1.0f); // 默认镜面反射
            material.shininess = 32.0f; // 默认光泽度
            material.diffuse_map = ""; // 默认没有纹理
        }
        else if (type == "Ka")
        {
            sin >> material.ambient.r >> material.ambient.g >> material.ambient.b;
        }
        else if (type == "Kd")
        {
            sin >> material.diffuse.r >> material.diffuse.g >> material.diffuse.b;
        }
        else if (type == "Ks")
        {
            sin >> material.specular.r >> material.specular.g >> material.specular.b;
        }
        else if (type == "Ns")
        {
            sin >> material.shininess;
        }
        else if (type == "map_Kd") // 漫反射纹理
        {
            sin >> material.diffuse_map;
        }
    }
...
Material* TriMesh::findMaterial(const std::string& name)
{
...
void TriMesh::readObj(const std::string& filename,int Ftype)
{
    ...
    while (std::getline(fin, line))
    {
        ...
        {
            sin >> _x >> _y >> _z;
            vertex_positions.push_back(glm::vec3(_x, _y, _z)); // 存储顶点坐标
        }
        // 解析OBJ文件中的法向量信息
        if (type == "vn")
        {
            sin >> _x >> _y >> _z;
            vertex_normals.push_back(glm::vec3(_x, _y, _z)); // 存储法向量
        }
        // 解析OBJ文件中的纹理坐标信息
        if (type == "vt")
        {
            sin >> _x >> _y >> _z;
            vertex_textures.push_back(glm::vec2(_x, _y)); // 存储纹理坐标
        }
        // 解析OBJ文件中的面信息
        if (type == "f" && Ftype == 1)
        {
            ...
            if (currentMaterial)
            {
                faceMaterials.push_back(currentMaterial);  // faceMaterials 用于存储每个面所用的材质
            }
        }
        else if (type == "f" && Ftype == 2)
        {
            sin >> a0 >> slash >> slash >> b0;
            sin >> a1 >> slash >> slash >> b1;
            sin >> a2 >> slash >> slash >> b2;
            // 存储面的顶点、纹理坐标和法向量的索引信息
            faces.push_back(vec3i(a0 - 1, a1 - 1, a2 - 1));
            normal_index.push_back(vec3i(b0 - 1, b1 - 1, b2 - 1));
        }
    }
...
glm::mat4 TriMesh::getSelfRotation() {
    return selfRotation;
}
void TriMesh::setShadowSphere(glm::vec3 _sphereCenter[],float _radius[]){
    for(int lo=1;lo < 5;lo++){
        sphereCenters[lo-1] = _sphereCenter[lo];
        sphereRadius[lo-1] = _radius[lo];
    }
...
glm::mat4 Light::getShadowProjectionMatrix() {
       // Y=-1平面上的阴影投影矩阵
        float lx, ly, lz;
        glm::vec3 light_position = this->getTranslation();
        lx = light_position[0];
        ly = light_position[1];
        lz = light_position[2];
    return glm::mat4(
            ly + 1.0f,    0.0f,      0.0f,      0.0f,
            -lx,          1.0f,     -lz,        -1.0f,
            0.0f,          0.0f,    ly + 1.0f,   0.0f,
            -lx,         -ly,      -lz,        ly
    );
