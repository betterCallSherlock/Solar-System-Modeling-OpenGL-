...
void MeshPainter::bindLightAndMaterial( TriMesh* mesh, openGLObject &object, Light* light, Camera* camera ) {
    ...
    // 传递相机的位置
    glUniform3fv(glGetUniformLocation(object.program, "eye_position"), 1, &camera->getPosition()[0]);
...
void MeshPainter::drawMesh(TriMesh* mesh, openGLObject &object, Light *light, Camera* camera){
	...
	camera->viewMatrix = camera->getViewMat();
    camera->projMatrix = camera->getProjMat();
    ...
    glUniform1f(glGetUniformLocation(object.program, "time"),glfwGetTime());
...
void MeshPainter::upDateMesh(TriMesh* mesh, openGLObject &object, Light *light, Camera* camera){
    ...
    // 物体的变换矩阵
    glm::mat4 modelMatrix = mesh->getModelMatrix();
    modelMatrix = modelMatrix*mesh->getSelfRotation();
    // 传递矩阵
    glUniformMatrix4fv(object.modelLocation, 1, GL_FALSE, &modelMatrix[0][0]);
    glUniformMatrix4fv(object.viewLocation, 1, GL_FALSE, &camera->viewMatrix[0][0]);
    glUniformMatrix4fv(object.projectionLocation, 1, GL_FALSE, &camera->projMatrix[0][0]);
...
void MeshPainter::updateMeshes(Light *light, Camera* camera,int loc){
        upDateMesh(meshes[loc], opengl_objects[loc], light, camera);
};
...
void MeshPainter::drawShadow(TriMesh *mesh, openGLObject &object, Light *light, Camera *camera) {
    ...
    glm::mat4 shadowMatrix = shadowProjMatrix * modelMatrix;
    // 传递矩阵
    glUniformMatrix4fv(object.modelLocation, 1, GL_FALSE, &shadowMatrix[0][0]);
    glUniformMatrix4fv(object.viewLocation, 1, GL_FALSE, &camera->viewMatrix[0][0]);
    glUniformMatrix4fv(object.projectionLocation, 1, GL_FALSE, &camera->projMatrix[0][0]);
    // 在渲染前设置阴影范围
    glUniform2f(glGetUniformLocation(object.program, "shadowRangeMin"), -1.0f*4, -1.0f*4);
    glUniform2f(glGetUniformLocation(object.program, "shadowRangeMax"), 1.0f*4, 1.0f*4);
    // 将着色器 isShadow 设置为1,表示阴影
...
void MeshPainter::drawShadows(Light *light, Camera *camera) {
    for (int i = 0; i < meshes.size(); i++)
    {
